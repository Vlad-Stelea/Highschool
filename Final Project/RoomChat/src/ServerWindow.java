...
public class ServerWindow extends JFrame {
	...
	public ServerWindow(int port,String userName) {
		...
		try {
			...
			JOptionPane.showMessageDialog(contentPane, "Could not start server");
			System.exit(1);
		}
		try {
		...
		Thread readMessages = new Thread(new Runnable(){
			...
			public void run() {
				for(;;){
				try {
					ArrayList<Message> newMes = cl.readMessage();
					...
					// TODO Auto-generated catch block
					JOptionPane.showMessageDialog(contentPane, "Lost connection to server");
					System.exit(1);
				}
				}
